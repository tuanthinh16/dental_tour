...
@section('title', $tour->translated('name').' | Dental Tour')
@section('meta_description', $tour->translated('short_description'))
@section('meta_keywords', $tour->translated('name').', '.($tour->destination?->name ?: 'Việt Nam').', '.$tour->destination?->name.', tour Việt Nam')
...
@section('content')
    <section class="grain ink-contrast relative flex min-h-screen items-end overflow-hidden bg-ink px-5 pb-20 pt-40 text-white md:px-10 md:pb-28">
        <div role="img" aria-label="{{ $tour->image?->alt_text ?: __('site.tour.details').' '.$tour->translated('name') }}" class="absolute inset-0 bg-cover bg-center opacity-75" style="background-image: url('{{ $tour->image?->file_path ?: 'https://picsum.photos/seed/'.urlencode($tour->slug).'/1920/1080' }}')"></div>
        ...
        <div class="relative mx-auto w-full max-w-[90rem]">
            <p class="text-sm font-medium text-coral">{{ $tour->destination?->name }} / {{ __('site.tour.experience') }}</p>
            <h1 class="display-type mt-6 max-w-6xl font-semibold">{{ $tour->translated('name') }}</h1>
            <div class="mt-10 grid gap-8 border-t border-white/25 pt-8 md:grid-cols-[1fr_auto_auto] md:items-end">
                <p class="max-w-2xl text-lg leading-8 text-white/65">{{ $tour->translated('short_description') }}</p>
                <div>
                    <p class="text-xs text-white/45">{{ __('site.tour.duration') }}</p>
                    <p class="mt-2 text-xl font-semibold">{{ __('site.tour.days', ['count' => $tour->duration_days]) }}{{ $tour->duration_nights ? ', '.__('site.tour.nights', ['count' => $tour->duration_nights]) : '' }}</p>
                </div>
                <div>
                    <p class="text-xs text-white/45">{{ __('site.tour.from_price') }}</p>
    ...
    <section class="bg-cream px-5 py-32 md:px-10 md:py-48">
        <div class="mx-auto grid max-w-[90rem] gap-16 lg:grid-cols-[0.7fr_1.3fr]">
            ...
            <div>
                <div class="whitespace-pre-line text-xl leading-9 text-ink/62">{{ $tour->translated('description') }}</div>
        ...
        <div class="mx-auto max-w-[90rem]">
            <div class="grid gap-14 lg:grid-cols-[0.65fr_1.35fr]">
                <div class="lg:sticky lg:top-36 lg:self-start">
                    <p class="text-sm font-medium text-coral">{{ __('site.tour.itinerary') }}</p>
    ...
    <section class="bg-mint px-5 py-32 md:px-10 md:py-48">
        <div class="mx-auto grid max-w-[90rem] gap-6 md:grid-cols-2">
            <div class="bg-white p-8 md:p-12">
                <h2 class="text-3xl font-semibold tracking-[-0.04em]">{{ __('site.tour.included') }}</h2>
                <ul class="mt-10 divide-y divide-ink/10">
                    @foreach($tour->services->where('is_active', true) as $service)
                        <li class="flex items-center gap-4 py-5 text-base"><span class="size-2 rounded-full bg-coral"></span>{{ $service->translated('name') }}</li>
                    @endforeach
                </ul>
            </div>
            <div class="bg-sand p-8 md:p-12">
                <h2 class="text-3xl font-semibold tracking-[-0.04em]">{{ __('site.tour.excluded') }}</h2>
                <ul class="mt-10 divide-y divide-ink/10">

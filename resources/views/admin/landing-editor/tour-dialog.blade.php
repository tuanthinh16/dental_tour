<div id="edit-tour-{{ $tour->id }}" data-visual-editor-panel hidden class="absolute inset-0 z-[60] overflow-y-auto bg-white/95 p-6 text-left text-ink backdrop-blur-md md:p-9">
    <form method="post" enctype="multipart/form-data" action="{{ route('admin.landing-editor.tours.update', $tour) }}">
        @csrf
        @method('PUT')
        <div class="flex items-start justify-between gap-5 border-b border-ink/10 pb-5"><div><h2 class="text-2xl font-semibold tracking-[-0.04em]">Sửa trực tiếp tour</h2><p class="mt-2 text-xs text-ink/45">{{ app()->getLocale() === 'en' ? __('admin.translation_notice') : 'Các trường có dấu * bắt buộc nhập.' }}</p></div><button data-visual-editor-close type="button" class="rounded-full border border-ink/15 px-4 py-2 text-xs font-semibold">Đóng</button></div>
        <div class="mt-6 grid gap-4 md:grid-cols-2">
            <label class="text-xs font-semibold md:col-span-2">Tên tour *<input name="name" value="{{ $tour->translated('name') }}" required class="mt-2 w-full border border-ink/15 bg-cream px-4 py-3 text-sm font-normal outline-none focus:border-coral"></label>
            @if(app()->getLocale() === 'en')<p class="-mt-2 text-xs text-ink/45 md:col-span-2">Giữ nguyên tên địa danh trong tiêu đề, ví dụ Phú Quốc hoặc Đà Nẵng.</p>@endif
            @if(app()->getLocale() === 'vi')
            ...
            @endif
            <label class="text-xs font-semibold md:col-span-2">Mô tả trên card *<textarea name="short_description" rows="3" required class="mt-2 w-full border border-ink/15 bg-cream px-4 py-3 text-sm font-normal outline-none focus:border-coral">{{ $tour->translated('short_description') }}</textarea></label>
            @if(app()->getLocale() === 'en')
                <label class="text-xs font-semibold md:col-span-2">Mô tả chi tiết tiếng Anh<textarea name="description" rows="6" class="mt-2 w-full border border-ink/15 bg-cream px-4 py-3 text-sm font-normal outline-none focus:border-coral">{{ $tour->translated('description') }}</textarea></label>
            @endif
            @if(app()->getLocale() === 'vi')
    ...
    @if(app()->getLocale() === 'vi')<form method="post" action="{{ route('admin.landing-editor.tours.destroy', $tour) }}" class="mt-4" onsubmit="return confirm('Xóa tour này?')">@csrf @method('DELETE')<button class="text-sm font-semibold text-red-600">Xóa tour</button></form>@endif
</div>

...
<div id="{{ $editorPanelId }}" data-visual-editor-panel hidden class="absolute inset-0 z-40 overflow-y-auto bg-white/95 p-5 text-left text-ink backdrop-blur-md md:p-7">
    <form method="post" enctype="multipart/form-data" action="{{ route('admin.landing-editor.destinations.update', $destination) }}">
        ...
        <div class="mt-4 grid gap-4">
            <label class="text-xs font-semibold">Tên điểm đến *<input name="name" value="{{ $destination->name }}" @readonly(app()->getLocale() === 'en') @required(app()->getLocale() === 'vi') class="mt-2 w-full border border-ink/15 bg-cream px-3 py-2 text-sm font-normal outline-none focus:border-coral"></label>
            @if(app()->getLocale() === 'en')<p class="-mt-2 text-xs text-ink/45">Tên địa danh giữ nguyên theo dữ liệu Việt gốc.</p>@endif
            <label class="text-xs font-semibold">Mô tả<textarea name="short_description" rows="3" class="mt-2 w-full border border-ink/15 bg-cream px-3 py-2 text-sm font-normal outline-none focus:border-coral">{{ $destination->translated('short_description') }}</textarea></label>
            @if(app()->getLocale() === 'vi')
    ...
    @if(app()->getLocale() === 'vi')<form method="post" action="{{ route('admin.landing-editor.destinations.destroy', $destination) }}" class="mt-3" onsubmit="return confirm('Xóa điểm đến này? Các tour liên quan sẽ không còn điểm đến.')">@csrf @method('DELETE')<button class="text-xs font-semibold text-red-600">Xóa điểm đến</button></form>@endif
</div>
